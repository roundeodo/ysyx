...
#(
  ...
  function automatic logic [31:0] merge_write_bytes(
    ...
    for (int unsigned byte_index = 0; byte_index < 4; byte_index++) begin
      if (byte_strobe[byte_index]) merged_data[byte_index*8+:8] = write_data[byte_index*8+:8];
    end
    return merged_data;
  endfunction
    ...
    unique case (read_state_q)
      READ_ACCEPT_ADDRESS: begin
        if (read_address_handshake) begin
          ...
          // CLINT registers are 32-bit, single-beat accesses. For an unsupported
          // burst the target still returns LEN+1 SLVERR beats, preserving AXI4.
          // 保留历史 RV64 配置的整宽 mtime 读取；本次新增中断验证限定 RV32。
...
`ifndef SYNTHESIS
  initial begin
    ...
    else $fatal(1, "CLINT requires an integer, nonzero clock divider");
  end
