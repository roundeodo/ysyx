...
// 读通道从展示 ARVALID 起锁定来源，直到 RLAST 握手完成；数据侧独占写通道。
module riscv32_axi4_arbiter
...
`ifndef SYNTHESIS
  ...
  assert property (@(posedge clk_i) disable iff (!rst_ni)
    ...
    $error("shared AXI read address changed while downstream was stalled");
  ...
  assert property (@(posedge clk_i) disable iff (!rst_ni)
    ...
    $error("shared AXI read requester changed before RLAST");
  ...
  assert property (@(posedge clk_i) disable iff (!rst_ni)
    ...
    $error("shared AXI read response was not routed to exactly one requester");

  a_instruction_and_data_read_responses_are_exclusive :
  assert property (@(posedge clk_i) disable iff (!rst_ni)
    !(instruction_manager_o.r_valid && data_manager_o.r_valid))
  else
    $error("shared AXI read response reached both requesters");
`endif
