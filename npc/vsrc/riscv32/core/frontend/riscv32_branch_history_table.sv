...
// 组合查询由父模块在请求握手沿采样；训练输入已经过共享解析事件寄存器。
// 同沿查询读取更新前的计数值，没有查询旁路。invalidate不清此表。
module riscv32_branch_history_table
  import riscv32_pkg::*;
#(
  ...
  // 00/01预测不跳，10/11预测跳转；父模块只为条件分支打开训练使能。
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      counter_array_q[training_index] <=
          update_counter(counter_array_q[training_index], training_taken_i);
    end
  end
endmodule
