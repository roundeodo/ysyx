...
module riscv32_icache_refill_axi4_master
  import riscv32_pkg::*;
...
#(
  ...
  assign axi_read_address_handshake =
      axi_manager_o.ar_valid && axi_manager_i.ar_ready;
  ...
  always_comb begin
    current_word_bit_offset =
        int'(current_word_addr_q[AXI_BYTE_OFFSET_WIDTH-1:0]) * 8;
  end
  ...
  always_comb begin
    ...
    unique case (state_q)
      ...
      REFILL_RECEIVE_READ_DATA: begin
        refill_resp_o.word_data = icache_fetch_data_t'(
            axi_manager_i.r.data >> current_word_bit_offset
        );
    ...
    unique case (state_q)
      REFILL_IDLE: begin
        if (refill_req_handshake) begin
          refill_transaction_context_d.requested_word_count =
              refill_req_i.requested_word_count;
          refill_transaction_context_d.transaction_index =
              refill_req_i.transaction_index;
      ...
      REFILL_RECEIVE_READ_DATA: begin
        if (axi_read_data_handshake) begin
          if (axi_manager_i.r.last) begin
            ...
            current_word_addr_d =
                current_word_addr_q + MEM_AXI_ADDR_WIDTH'(ICACHE_FETCH_BYTES);
          end
        end
      end
