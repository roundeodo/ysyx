...
(
  ...
  assign execute_issue_allowed_o    = execute_progress_allowed_o &&
      ...
      !commit_redirect_event_i;

  assign decode_execute_flush_o = execute_redirect_present_i || control_recovery_event;
  assign writeback_flush_o      = commit_redirect_event_i;
...
`ifndef SYNTHESIS
  ...
  assert property (@(posedge clk_i) disable iff (!rst_ni)
    ...
    $error("pipeline accepted a decoded instruction with an unresolved RAW hazard");

  assert property (@(posedge clk_i) disable iff (!rst_ni)
    control_recovery_event |-> decode_execute_flush_o)
  else
    $error("pipeline recovery did not flush the ID/EX stage");

  assert property (@(posedge clk_i) disable iff (!rst_ni)
    execute_redirect_present_i |-> !execute_issue_allowed_o)
  else
    $error("pipeline allowed a younger instruction to issue during branch recovery");

  assert property (@(posedge clk_i) disable iff (!rst_ni)
    (lsu_busy_i && !lsu_completion_succeeded_i) |-> !execute_issue_allowed_o)
  else
    $error("pipeline issued a younger instruction ahead of an older LSU transaction");
  ...
  assert property (@(posedge clk_i) disable iff (!rst_ni) $onehot0(
    ...
    $error("pipeline selected multiple forwarding producers for one source operand");

  assert property (@(posedge clk_i) disable iff (!rst_ni)
    (decoded_uop_valid_i && execute_blocking_writes_rs1)
    |-> raw_hazard_present_o)
  else
    $error("pipeline accepted stale rs1 data while the newest EX producer was unavailable");
