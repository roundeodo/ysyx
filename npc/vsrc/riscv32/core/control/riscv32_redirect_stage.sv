...
(
  ...
  redirect_req_t redirect_req_array_q[SOURCE_COUNT];
  ...
  assign registered_redirect_req_o = registered_redirect_req_valid_q ?
      redirect_req_array_q[selected_source_index_q] : '0;
    ...
    if (!rst_ni) begin
      ...
      for (int unsigned source = 0; source < SOURCE_COUNT; source++) begin
        if (redirect_req_valid_i[source]) begin
          redirect_req_array_q[source] <= redirect_req_i[source];
...
`ifndef SYNTHESIS
  assert property (@(posedge clk_i) disable iff (!rst_ni)
    ...
    $error("frontend redirect register lost a recovery event");

  assert property (@(posedge clk_i) disable iff (!rst_ni)
    !(|redirect_req_valid_i) |=> !registered_redirect_req_valid_o)
  else
    $error("frontend redirect register created a recovery event");
`endif
