...
//
// 当前检查点每次只处理一个lookup或miss，控制面积并保证精确异常路径清晰；同步tag/data
// array、独立miss unit和line AXI adapter是长期边界。面向AI负载扩展时，应在这些边界上
// 增加bank、多MSHR、writeback queue和store buffer，而不是把AXI塞回LSU。
...
(
  ...
  function automatic dcache_set_index_t get_set_index(input phys_addr_t addr);
    if (DCACHE_SET_INDEX_BITS == 0) return '0;
  ...
  function automatic dcache_word_index_t get_word_index(input phys_addr_t addr);
    if (DCACHE_WORD_INDEX_BITS == 0) return '0;
  ...
  dcache_way_index_t replacement_next_way_q[DCACHE_SET_COUNT];
  logic line_install_occurred;
  dcache_set_index_t installed_set_index;
  dcache_way_index_t installed_way_index;

  always_comb begin
    replacement_way_index = replacement_next_way_q[get_set_index(lookup_s1_q.addr)];
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    ...
    end else if (line_install_occurred) begin
      replacement_next_way_q[installed_set_index] <=
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) clean_state_q <= CLEAN_IDLE;
    else clean_state_q <= clean_state_d;
  end
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) clean_access_fault_q <= 1'b0;
    else clean_access_fault_q <= clean_access_fault_d;
  end
  ...
  riscv32_dcache_miss_unit u_miss_unit (
      ...
      .line_install_occurred_o         (line_install_occurred),
  ...
  riscv32_dcache_line_axi4_master u_line_axi4_master (
      .clk_i                    (clk_i),
      .rst_ni                   (rst_ni),
  ...
  assign store_hit_write_occurred = local_resp_handshake &&
  ...
  always_ff @(posedge clk_i) begin
    if (lookup_req_handshake) lookup_s1_q <= data_memory_req_i;
  end

  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) lookup_s1_present_q <= 1'b0;
    else lookup_s1_present_q <= lookup_s1_present_d;
  end
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      store_write_bypass_present_q <= store_hit_write_occurred && lookup_req_handshake;
      if (store_hit_write_occurred && lookup_req_handshake) begin
        store_write_bypass_set_index_q <= get_set_index(lookup_s1_q.addr);
  ...
  always_comb begin
    ...
    if (!miss_data_write_valid && store_hit_write_occurred) begin
  ...
  always_comb begin
    ...
    if (!miss_metadata_write_valid && store_hit_write_occurred) begin
  ...
  always_comb begin
    ...
    event_o.store_hit_and_next_lookup_occurred      = store_hit_write_occurred &&
                                                      lookup_req_handshake;
    event_o.miss_occurred                           = miss_req_handshake;
    event_o.dirty_victim_miss_occurred              = miss_req_handshake &&
                                                      miss_req.victim_present &&
                                                      miss_req.victim_dirty;
    event_o.refill_request_occurred                 = refill_req_valid && refill_req_ready;
    event_o.refill_word_occurred                    = refill_resp_valid && refill_resp_ready;
    event_o.refill_transaction_completed_occurred   = refill_resp_valid &&
                                                      refill_resp_ready &&
                                                      refill_resp.last_word;
    event_o.line_install_occurred                   = line_install_occurred;
    event_o.writeback_request_occurred              = writeback_req_valid &&
                                                      writeback_req_ready;
    event_o.writeback_response_occurred             = writeback_resp_valid &&
...
`ifndef SYNTHESIS
  initial begin
    assert (DCACHE_CAPACITY_BYTES > 0)
      else $fatal(1, "D-cache capacity must be positive");
    assert (DCACHE_WAY_COUNT > 0)
      else $fatal(1, "D-cache way count must be positive");
    assert ((DCACHE_LINE_BYTES & (DCACHE_LINE_BYTES - 1)) == 0)
      else $fatal(1, "D-cache line bytes must be a power of two");
    assert ((DCACHE_SET_COUNT & (DCACHE_SET_COUNT - 1)) == 0)
      else $fatal(1, "D-cache set count must be a power of two");
  end

  assert property (@(posedge clk_i) disable iff (!rst_ni)
    data_memory_req_valid_i && !data_memory_req_ready_o |=>
      (data_memory_req_valid_i && $stable(data_memory_req_i)))
  else $error("D-cache request changed while stalled");

  assert property (@(posedge clk_i) disable iff (!rst_ni)
    data_memory_resp_valid_o && !data_memory_resp_ready_i |=>
      (data_memory_resp_valid_o && $stable(data_memory_resp_o)))
  else $error("D-cache response changed while stalled");

  assert property (@(posedge clk_i) disable iff (!rst_ni)
    $onehot0(lookup_way_hit_vector))
  else $error("D-cache found the same line in multiple ways");

  assert property (@(posedge clk_i) disable iff (!rst_ni)
    lookup_req_handshake |-> (tag_read_enable && data_read_enable))
  else $error("D-cache accepted a lookup without sampling both arrays");
`endif
