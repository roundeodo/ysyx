...
//
// 当前只有一个事务上下文，但接口把lookup、阵列和line AXI协议分开。后续增加多个
// MSHR时应把本上下文扩展成队列，而不是让LSU或AXI ID承担cache内部状态。
...
(
    ...
    output logic               line_install_occurred_o,
  ...
  always_comb begin
    ...
    if (writeback_resp_handshake) begin
      ...
      if (writeback_resp_i.access_fault) begin
        if (clean_operation_q) clean_access_fault_d = 1'b1;
        else begin
    ...
    unique case (state_q)
      ...
      MISS_SEND_REFILL: begin
        if (refill_req_handshake) state_d = MISS_RECEIVE_REFILL;
      end
      ...
      MISS_RETURN_RESPONSE: begin
        if (miss_resp_valid_o && miss_resp_ready_i) state_d = MISS_IDLE;
      end
...
`ifndef SYNTHESIS
  ...
  else $error("D-cache miss and clean requests were presented together");

  assert property (@(posedge clk_i) disable iff (!rst_ni)
    metadata_write_line_dirty_o |-> metadata_write_line_present_o)
  else $error("D-cache attempted to create a dirty invalid line");

  assert property (@(posedge clk_i) disable iff (!rst_ni)
    refill_resp_handshake && refill_resp_i.last_word |->
      (refill_resp_i.word_index == dcache_word_index_t'(DCACHE_WORDS_PER_LINE - 1)))
  else $error("D-cache refill RLAST arrived at an unexpected word index");

  assert property (@(posedge clk_i) disable iff (!rst_ni)
    state_q == MISS_INSTALL_LINE |->
      (!writeback_response_pending_q && !transaction_access_fault_q))
  else $error("D-cache installed a line before all bus responses completed successfully");
