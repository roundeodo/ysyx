...
module riscv32_dcache_line_axi4_master
  import riscv32_pkg::*;
...
#(
  ...
  always_comb begin
    ...
    unique case (refill_state_q)
      ...
      REFILL_AXI_SEND_ADDRESS: begin
        if (axi_read_address_handshake) refill_state_d = REFILL_AXI_RECEIVE_DATA;
      end
  ...
  always_comb begin
    ...
    unique case (writeback_state_q)
      ...
      WRITEBACK_AXI_SEND_ADDRESS_DATA: begin
        if (axi_write_address_handshake) write_address_pending_d = 1'b0;
        if (axi_write_data_handshake) begin
      ...
      WRITEBACK_AXI_RECEIVE_RESPONSE: begin
        if (axi_write_response_handshake) writeback_state_d = WRITEBACK_AXI_IDLE;
      end
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) refill_state_q <= REFILL_AXI_IDLE;
    else refill_state_q <= refill_state_d;
  end

  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) writeback_state_q <= WRITEBACK_AXI_IDLE;
    else writeback_state_q <= writeback_state_d;
  end
...
`ifndef SYNTHESIS
  initial begin
    assert (CORE_DATA_WIDTH == MEM_AXI_DATA_WIDTH)
      else $fatal(1, "D-cache word and memory AXI beat widths must match");
    assert ((DCACHE_LINE_BYTES % DCACHE_WORD_BYTES) == 0)
      else $fatal(1, "D-cache line must contain an integer number of AXI beats");
    assert (DCACHE_WORDS_PER_LINE <= 256)
      else $fatal(1, "AXI4 burst length exceeds 256 beats");
  end
