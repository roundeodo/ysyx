...
module riscv32_sim_icache_monitor
  import riscv32_pkg::*;
(
    input  logic clk_i,
    input  logic rst_ni,
  ...
  assign lookup_response_available_for_active_request =
      ...
      (lookup_timer_active_q || icache_event_i.lookup_event);
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      lookup_count_q      <= lookup_count_q + 64'(icache_event_i.lookup_event);
      miss_count_q        <= miss_count_q + 64'(icache_event_i.miss_event);
      refill_word_count_q <= refill_word_count_q +
          64'(icache_event_i.refill_word_event);
      refill_line_count_q <= refill_line_count_q +
          64'(icache_event_i.refill_line_completed_event);
      uncached_access_count_q <= uncached_access_count_q +
          64'(icache_event_i.uncached_access_event);
      stale_response_count_q <= stale_response_count_q +
          64'(icache_event_i.stale_response_discarded_event);
      ...
      if (icache_event_i.miss_event) begin
      ...
      if (lookup_response_available_for_active_request) begin
        ...
        end else if (lookup_classified_as_miss_q || icache_event_i.miss_event) begin
      ...
      // lookup_event启动新计时器并清除此标志。这样连续命中即使valid不降也能逐事务计数。
      // 只有在“本拍新请求本拍产生响应”这种零延迟接口情况下，才不留下活动计时器。
      if (icache_event_i.lookup_event &&
      ...
      if (icache_event_i.refill_transaction_completed_event) begin
  ...
  always_ff @(posedge clk_i) begin
    if (rst_ni) begin
      assert ((hit_count_q + miss_count_q + uncached_access_count_q) <= lookup_count_q)
        ...
        $error("I-cache classified more requests than it accepted");

      assert (!icache_event_i.lookup_response_is_cache_hit ||
              icache_event_i.lookup_response_present)
      else
        $error("I-cache marked a response as a hit while no response was present");

      assert (refill_line_count_q <= miss_count_q)
      else
        $error("I-cache completed more refill lines than cacheable misses");

      assert (miss_response_count_q <= miss_count_q)
      else
        $error("I-cache returned more miss responses than allocated misses");

      assert (refill_transaction_count_q <= miss_count_q)
      else
        $error("I-cache finished more refill transactions than allocated misses");

      assert (refill_line_count_q <= refill_transaction_count_q)
      else
        $error("I-cache installed more lines than completed refill transactions");

      assert (refill_word_count_q >= (refill_line_count_q * ICACHE_WORDS_PER_LINE))
      else
        $error("I-cache completed a refill line without receiving all words");

      assert (!(icache_event_i.lookup_event && lookup_timer_active_q &&
                !icache_event_i.lookup_response_present))
      else
        $error("I-cache AMAT timer observed more than one outstanding lookup");

      assert (!icache_event_i.miss_event || lookup_timer_active_q ||
              icache_event_i.lookup_event)
      else
        $error("I-cache AMAT timer observed a miss without an active lookup");

      assert (!icache_event_i.refill_transaction_completed_event ||
              refill_timer_active_q)
      else
        $error("I-cache refill timer observed completion without an active refill");
    end
  end
