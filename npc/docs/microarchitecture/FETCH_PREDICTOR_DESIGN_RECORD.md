...
当前实现，2026-09-16。查询只有一个响应寄存级；与 IFU 的连接见
...
| 模块 | 拥有的状态与功能 |
| --- | --- |
| `riscv32_branch_predictor` | 共享训练事件寄存器；并行查询结果选择；唯一响应寄存器及统一握手 |
| `riscv32_bht` | 两位饱和计数器数组、组合查询、条件分支同步训练 |
| `riscv32_btb` | tag/target/kind、有效位、替换指针；组合查询与训练选路、同步写入 |
| `riscv32_ras` | 解析栈操作寄存器、返回地址数组、写指针、数量；组合读取栈顶 |

父模块源码先列训练入口，再列共享的三个表、查询选择与响应寄存器。
训练和查询是并行路径，BHT、BTB、RAS 之间没有串联寄存级。
BHT 按 PC 索引两位饱和计数器；BTB 按组并行比较各路 tag；RAS 保存已解析调用的返回地址。
...
当前 IFU 接收 taken 响应时更新 PC，下一拍查询目标，连续命中跳转的目标请求间隔为两拍。
这是前端接口间隔，不等于每次分支的退休停顿。

## 训练时序与清除

训练来源为无异常控制流执行结果被接收的事件。E0 保存 PC、target、kind、taken；
RAS 并行保存栈操作与返回地址。E1 更新三个单元。训练并不等待 commit。
BTB 当拍直接读取当前数组，优先同 tag，再空 way，最后轮转替换；连续同 set 写入不需要
待写事务旁路。查询和训练同沿时仍使用旧表值，不把 EX 结果组合旁路到查询端。

复位使 BHT 计数器为 01，清 BTB 有效位和替换指针，清 RAS 数量与指针。
invalidate 清 BTB、RAS 和待处理事件；BHT 保留计数器，并可在该沿完成此前的训练。
...
训练寄存器隔开执行级与表写入路径；RAS 只根据已解析结果更新，不需要推测检查点，
...
- 全核、缓存、计时和 PPA 结果见 [前端重写验证](../verification/RV32_FRONTEND_REWRITE_2026-09-15.md)。
