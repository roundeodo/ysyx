> 历史归档，停止更新。当前实现见[现行 I-cache 说明](../ICACHE_DESIGN_RECORD.md)。

# I-cache设计记录

2026-09-15 当前实现补充：前端重写移除了阻塞协议下不可达的回填命中通路及 miss 进度输出。
关键 word 仍由 miss unit 提前返回；I-cache 在整行回填及待消费 miss 响应结束前拒绝新请求。
同步阵列读和对齐请求级保留。见 [前端重写记录](../FRONTEND_REWRITE_DESIGN_RECORD.md)。
