...
当前实现，2026-09-16。阅读入口；预测器和 I-cache 的内部细节分别在各自说明中维护。

## 取指怎样完成

1. **PC 与预测**：IFU 发出 PC；BHT、BTB、RAS 并行组合读取，预测器用一个响应寄存器
   保存 PC、epoch 和预测结果。IFU 接收预测后，把请求送入两项请求队列。
2. **请求队列与 I-cache**：队首向 I-cache 发起查询。I-cache 同步读取 tag/data，
   命中直接返回；未命中由单个 miss 单元完成回填，关键指令字可以提前返回。
3. **响应与译码**：IFU 按返回 tag 取回预测信息，丢弃旧 epoch 响应；有效指令写入两项
   fetch buffer，随后交给译码。fetch buffer 无空队列组合穿透。

预测训练是独立的反馈路径：执行结果先寄存，下一沿更新 BHT、BTB、RAS。
...
| IFU 两项请求队列 | 等待 I-cache 接收的地址、身份、预测 | 缓冲请求，隔开 I-cache 与预测请求的 ready 路径 |
| IFU 按 tag 保存的预测表 | 已被 I-cache 接收的请求对应的预测 | 在指令返回时重新配对 |
...
返回后按 epoch 丢弃；不能中途撤销被反压的 valid。
...
功能、面积、频率和性能数据只在[验证记录](../verification/RV32_FRONTEND_REWRITE_2026-09-15.md)
维护；这里不复制结果表或后续架构计划。
