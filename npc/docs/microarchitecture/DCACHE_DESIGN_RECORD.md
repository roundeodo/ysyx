...
安装新行前必须确认 B 和全部 R 均无错。脏替换遇到 B/R 错误时，先排空已发出的响应，
再进入 RESTORE_VICTIM，复用整行 victim 缓冲和字索引逐字写回原位置；最后一字同时恢复
旧 tag、present、dirty，再向当前访问返回 access fault。恢复前不释放事务，不允许新 miss
覆盖缓冲。错误路径多用每行字数个周期，正常路径不新增等待，也不增加第二个整行缓冲。
即使下层只保存了部分写回数据，本核仍从恢复的完整脏行读数，后续可以重新写回。
这不是总线写入回滚，也不承诺多 hart/DMA 一致性。clean 失败不覆盖数据，保留 dirty；
FENCE.I 控制器将该错误按不可恢复维护错误处理。
