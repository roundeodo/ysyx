...
- [2026-09-22～23：分支预测研究](BRANCH_RESEARCH_NOTES.md)：边缘 AI 软件控制流、近期与经典论文、
  BTB/方向/返回问题分解，模型、RTL、PPA、保留集及保留基线的最终取舍。

- [2026-09-22：RV32 前端研究、实现与取舍](FRONTEND_RESEARCH_NOTES.md)：论文和项目阅读顺序、
  当前分段访问 RRIP 的研究来源、查询旁路的电路依据、模型/RTL 对照、负结果与后续分支研究入口。
  完整测量仍由对应验证记录维护，不在日志中再复制一份。
...
## 2026-09-15：先确定前端电路结构，再按寄存器边界组织 RTL

本轮明确了代码顺序：训练事件入口、并行查表、预测选择、响应缓冲；有状态机的模块
依次写输出、下一状态和时序更新。模块边界不要求流水边界，数组状态与查询快照也不能混为一谈。

取消预测器中间查询级与 BTB 两级训练状态、RAS 栈顶副本，并清理 blocking I-cache
不可达的回填命中路径后，当前综合面积从 76,480.320 降至 73,030.034 μm²，DFF 减少 459 个。
真实 IFU 的连续 taken 请求间隔由三拍缩到两拍，同频 MicroBench test 的 Total 周期减少 2.52%。
同时估算 Fmax 从 742.966 降至 707.409 MHz，说明寄存器减少、IPC 上升都不单独保证用时缩短。
采用通过 STA 的 700 MHz 配置，实际用时仍以该配置下的原生 timer 结果为准。

这些结果支持此次“小表组合读 + 单个响应缓冲”的取舍，不证明更大预测表也应采用相同级数。
[完整设计与验证](../verification/RV32_FRONTEND_REWRITE_2026-09-15.md)。

本轮 700 MHz 原生短测最终通过：Total 0.006426 s / IPC 0.169737606，Scored
0.002904 s / IPC 0.211616786。它与 750 MHz 同频结构对照分开记录，避免把设备相对延迟
变化产生的 IPC 差值归为硬件优化收益。
