...
结构复核日期：2026-09-19。对象为 `ysyx-workbench-rv32-interview` 工作树、
`NPC_CONFIG=rv32-baseline`，包含定时中断、时序修复、译码级合并和 LSU 成功完成拍发射优化，
现已取消独立 RR，并将两项取指缓冲改为环形 FIFO。预测器内部的 BHT、BTB、RAS 已拆为
独立模块；随后前端重写将查询缩为单级，BTB 取消训练快照和待写事务。只讨论 RV32，
历史实验按版本标注。本轮加入访存与队列直通、load 前递，删除 ID/EX skid 和恢复寄存级；
当前面积、时序与 IPC 见[周期优化验证](../verification/RV32_CYCLE_OPT_2026-09-19.md)。
取消 RR 的实现与当时测量见 [取消 RR 的实现记录](RV32_RR_REMOVAL_2026-09-15.md)。
首次预测器模块拆分的历史网表为 76,480.320 μm²，730 MHz 时序检查通过，详见
...
最新前端变化及测量见[周期优化验证](../verification/RV32_CYCLE_OPT_2026-09-19.md)。
...
| 取指缓冲采用两项环形 FIFO | 出队只改读指针，消除译码/冒险控制到整条指令搬移选择的路径 | 保留两项容量，空队列可直通、满队列出队可同拍入队；减少传递等待，代价是 ready 组合路径变长 |
...
| BHT、BTB、RAS 各自独立，统一预测控制 | 将数组、更新和断言归属到对应模块，便于局部阅读与测试；父模块保证 PC、方向与目标同拍对应 | 首次拆分保持行为；随后重写取消中间级，并使用独立模型验证新的延迟和训练契约，详见预测器设计记录 |
| IFU lookup 队列 2 项，预测查询单级 | 表项读取和预测选择在同一周期完成，只保留完整响应寄存器；队列吸收 I-cache 反压 | 正确 taken 的预测目标查询间隔进一步缩至一拍；组合路径变长，频率须实测 |
...
[IFU](../../vsrc/riscv32/core/frontend/riscv32_ifu.sv)、
[预测器](../../vsrc/riscv32/core/frontend/riscv32_branch_predictor.sv)。
...
三个子模块并行读取同一个请求 PC 对应的方向、目标和当前栈顶。父模块组合选择后，
在一次握手沿保存 PC、epoch 和完整预测。响应反压与 flush 由父模块统一处理。
不再保存 BHT/RAS 和 BTB set 的中间查询快照。
...
请求发出前必须排除更老指令尚未解决的控制/异常风险。当前 EX 结果级有有效异常时，
即使它能够进入 WB，也会阻止年轻指令发射，避免 store 先进入 LSU；下一拍 WB 统一恢复。
老 LSU 未完成时也阻止年轻执行。store 并非都到 commit 才写总线，已接受的写事务不能
靠 flush 撤销；涉及设备错误时，还要说明目标的响应语义和已完成副作用边界。
