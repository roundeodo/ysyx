...
历史实验按版本标注。当前面积与时序见[精确异常修复验证](../verification/RV32_PRECISE_EXCEPTION_FIX_2026-09-19.md)；
...
最新前端变化及测量见 [前端重写验证](../verification/RV32_FRONTEND_REWRITE_2026-09-15.md)。
...
| IFU lookup 队列 2 项，预测查询单级 | 表项读取和预测选择在同一周期完成，只保留完整响应寄存器；队列吸收 I-cache 反压 | 正确 taken 的预测目标间隔由三拍缩至两拍；组合路径变长，频率须实测 |
