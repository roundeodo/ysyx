...
## 2026-09-15：前端按电路结构重写，减少查询与训练寄存器

开发基线 `d8bb7dd`；工作树原有的 BTB 注释/格式修改保存在本轮 `before.patch`。
先更新编码手册、流程、模块模板和前端设计记录，再实现单级预测查询、直接 BTB 训练、
无栈顶副本的 RAS；删除严格 blocking I-cache 中不可达的 refill hit 逻辑。
IFU、fetch FIFO 和 cache 控制按输出/下一值/时序组织，外部 cache/AXI 事务保持原有契约。

验证：RV32 lint、流水线、5 配置 60,000 周期独立预测模型、100 次真实 IFU taken 交付通过；
I-cache/AXI 一路 16 B line、两路 4 B line 通过，另以 64-bit AXI 配置检查 byte lane。
3 汇编 + 2 AM 中断系统测试、3 RTL 单元测试、35 项 RV32 CPU DiffTest 全部通过。
旧拆分等价脚本改为明确回放两个冻结提交，并通过回放；当前单级实现用独立模型验证。

同频 750 MHz MicroBench test 十项 PASS、GOOD TRAP：Total 周期 4,828,640 → 4,706,864，
IPC 0.158123405 → 0.162214375，原生时间 0.006438 → 0.006276 s；Scored 周期
2,168,036 → 2,115,877，IPC 0.198480560 → 0.203373353，原生时间 0.002891 → 0.002824 s。
此频率仅作相同访存时序比例下的 RTL 对照。

保持 820 MHz 综合目标与 DELAY 0：缓冲网表面积 73,030.034 μm²，DFF 8,410，
估算 Fmax 707.409 MHz。相同网表在 700 MHz 的 setup/hold/gating setup/gating hold
分别为 +0.014/+0.057/+0.115/+0.091 ns，通过检查。
所有原始证据位于 `npc/result/performance/frontend-rewrite-20260915/`，
具体命令、界限与后续 700 MHz 原生计时结果见
[前端重写验证](RV32_FRONTEND_REWRITE_2026-09-15.md)。未运行完整 train。

700 MHz 原生短测补充：十项 PASS、GOOD TRAP；Total 4,498,243 cycles / 763,521 retired /
IPC 0.169737606 / timer 0.006426 s；Scored 2,033,454 cycles / 430,313 retired /
IPC 0.211616786 / timer 0.002904 s。两次软件测量归档的前端 RTL 均与当前文件哈希一致。
旧版 730 MHz 原生短测未运行，不据此宣称两版各自有效频率下的时间已改善。

## 2026-09-16：前端可读性复查

确认 12 个前端/IFU 文件与 9 月 15 日测量归档哈希一致后，整理排版和普通注释。
补充人工可读性验收；修正 IFU 单级查询、阵列读地址来源与 RLAST 转发等旧注释。
Verible 语法检查及修改前后完整词法单元比较全部通过，全核 lint 与 `git diff --check`
通过；未改变 RTL 逻辑，未重跑综合或工作负载。原测量哈希保持原样，当前文本的对应证据见
[验证记录补充](RV32_FRONTEND_REWRITE_2026-09-15.md#2026-09-16可读性复查)。

同日补齐 IFU 与 I-cache 的源码阅读顺序并精简模块说明。完整模块项语法树比较、lint、
IFU 100 次跳转交付及 I-cache 13 项检查通过；未改硬件逻辑，详见上述验证记录补充。

## 2026-09-16：后端与系统代码可读性整理

基于 `d8bb7dd` 与已有前端改动的未提交工作树。梳理 47 个现有非前端 SV 文件，
将组合操作数准备、FENCE.I 状态归入两个独立模块，整理 D-cache、执行、总线、CSR/PMU
和 CLINT 的源码顺序。结构核对保留了原有运算与更新过程，没有新增流水级。
RV32 的 9 个模块检查入口、35 个整核 DiffTest 程序和 8 项定时中断回归通过；
RV64 共享转换器/特权检查、NPC/SoC lint 及综合文件清单静态检查通过。
初次测试配置错误与纠正、准确命令及源码哈希见[本轮验证记录](RV32_READABILITY_2026-09-16.md)。
本轮未重测面积、频率或 microbench train。


## 2026-09-19：完整 riscv32 目录整理与分层

补齐全部 67 个文件的检查，按流水线功能迁移 21 个模块；编码规范、构建入口和当前文档同步。
整核 DiffTest 35/35、定时中断 8/8 及模块回归通过；移动后的内容核对和编译检查通过。
详见[完整目录检查记录](RV32_FULL_DIRECTORY_REVIEW_2026-09-19.md)，本轮未重新测量 PPA 或 train。

## 2026-09-19：整理后的综合与性能复测

实际综合发现冒险控制模块先使用后声明，已修复；同时纠正两处性能报告文字。
同工具、参数和约束下，面积 73,030.034 → 73,037.748 μm²，估算 Fmax 707.409 →
712.892 MHz，700 MHz 最差 setup slack +0.014 → +0.025 ns，hold 和门控检查通过。
microbench test 的总周期、退休指令、全部采样和性能计数与前端重写基线相同：
Total IPC 0.169737606，原生定时器时间 6.426 ms。未重新运行 train。
源码、修复范围、时序边界及复现命令见[复测记录](RV32_READABILITY_PPA_2026-09-19.md)。

## 2026-09-19：模块边界复核与名称简化

按状态归属、协议和组合选择职责复核后，保留现有模块边界；理由维护在[模块说明](../microarchitecture/README.md#为什么保留这些模块边界)。
缩短 23 个模块名，同步文件名、实例名、构建入口、测试及当前文档；历史实验保留原名称。
[命名映射与验证证据](../../result/readability/rv32-module-names-20260919/)保存前后快照。
67 个文件通过逐字节核对：只替换明确列出的标识符与路径，表达式、寄存边界、源码顺序均相同。
Yosys/Slang 读取综合清单并检查通过；RV32 NPC/SoC、RV64 共享 RTL lint 通过。
预测器 5 配置/60,000 周期、取指、I-cache、流水控制、I/D 仲裁、D-cache，以及复位和执行结果
寄存器回归均通过。本轮未重新测量 PPA 或 microbench；前一节数值对应改名前的源码快照。

## 2026-09-19：未使用模块移出当前设计目录

按 NPC、SoC 接口和综合三个顶层核对 59 个模块的源码引用；52 个可达（包含条件仿真实例），
其余为 3 个已有独立实验和 4 个未接入模块。将旧 decode/RR 寄存级、旧组合重定向仲裁移入
`experiments/pipeline/`，独立 AXI 错误目标移入 `experiments/interconnect/`；从两份活动清单
移除这 4 个文件，并清除流水控制测试多余的 RR 文件依赖。当前模块和历史模块的区别见
[模块说明](../microarchitecture/README.md#不属于当前-cpu-的模块)。

65 个 RTL/header 文件内容全部保持不变；清理前后 `riscv32_core_reset_boundary` 的
Yosys/Slang RTLIL 逐字节相同。RV32 NPC/SoC lint、RV64 NPC lint、重新编译的流水控制
...
## 2026-09-19：寄存器与额外周期审查

对象为 `c253a1a8fdc168ffaab4e7ea1713617a815d6346`，RV32 baseline，未修改生产 RTL。
复核 45 个模块与 5 个 package，结合已有映射网表确认 IFU 预测表和 D-cache 双写回
缓冲等状态开销，列出 LSU 交接、I-cache AR 入口及 D-cache miss 状态合并候选。
独立 D-cache 定向测试复现：响应反压时，改变无效请求端口地址会改变旧响应数据；
保持地址的控制用例通过。尚未证明当前整核程序触发，不能把模块复现扩大为整核失败。
[审查记录](RV32_REGISTER_CYCLE_REVIEW_2026-09-19.md)包含完整范围、取舍及证据。
本轮没有新的面积、频率或 IPC 测量，也未将候选优化计为已实现。


## 2026-09-19：落实直通、同拍交接与冗余状态优化

以 `c253a1a8` 为基线，修复 D-cache 响应反压保持，落实 IFU 单上下文、队列直通、
LSU/AXI 空闲直通和完成拍接收、load 前递、miss 状态合并、单份脏行缓存、直接训练与组合恢复。
保留指令年龄、同步阵列和未完成总线事务需要的状态；没有改变缓存/预测器容量或外设时间模型。
最终 32 项精确异常、35 项 DiffTest、8 项中断回归通过，完整系统位级检查无组合环。
面积从 73,081.372 降至 69,580.014 μm²；700 MHz 同频 test IPC 从 0.169738 提高至 0.190168。
采用通过时序检查的 600 MHz，原生 Total 为 6.005 ms、IPC 0.212130，比基线 700 MHz 用时短 6.55%。
这是 test 数据，未将其用于代替 train。状态边界、频率取舍、精确证据与 train 命令见
[完整验证记录](RV32_CYCLE_OPT_2026-09-19.md)。
