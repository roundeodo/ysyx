...
(
    ...
    input  logic          issue_event_i,
    input  logic          execute_valid_i,
    input  logic          redirect_event_i,
  ...
  assign recovery_present = recovery_pending_q || redirect_event_i;

  always_comb begin
    category_index = 7;
    if (issue_event_i)
      category_index = 0;
    else if (recovery_present)
      category_index = 1;
    else if (lsu_busy_i)
      category_index = 2;
    else if (execute_result_blocked_i)
      category_index = 3;
    else if (execute_valid_i)
      category_index = 4;
    else if (register_read_valid_i && raw_hazard_present_i)
      category_index = 5;
    else if (register_read_valid_i && serializing_hazard_present_i)
      category_index = 6;
  end
  ...
  always_ff @(posedge clk_i) begin
    if (!rst_ni) begin
      ...
      if ((window_active_q && !cycle_read_present) ||
        ...
        if (redirect_event_i)
          redirect_count_q <= redirect_count_q + 1;
        if (recovery_present && !issue_event_i)
          recovery_cycle_count_q <= recovery_cycle_count_q + 1;
        if (lsu_busy_i)
          lsu_busy_cycle_count_q <= lsu_busy_cycle_count_q + 1;
        if (icache_event_i.miss_event)
          icache_miss_count_q <= icache_miss_count_q + 1;
        if (dcache_event_i.miss_event)
          dcache_miss_count_q <= dcache_miss_count_q + 1;
        if (dcache_event_i.dirty_victim_miss_event)
          dirty_miss_count_q <= dirty_miss_count_q + 1;
      end
