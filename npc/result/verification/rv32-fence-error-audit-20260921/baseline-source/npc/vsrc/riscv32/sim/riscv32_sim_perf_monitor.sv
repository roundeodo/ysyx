...
module riscv32_sim_perf_monitor
  import riscv32_pkg::*;
(
    input  logic clk_i,
    input  logic rst_ni,

    input  logic ifu_instruction_request_event_i,
    input  logic ifu_instruction_request_waiting_i,
    input  logic ifu_instruction_response_event_i,
    input  logic ifu_instruction_response_discarded_event_i,
    input  logic ifu_instruction_delivery_event_i,
    input  logic ifu_downstream_waiting_i,
    input  logic icache_miss_event_i,

    input  logic     instruction_decode_event_i,
    input  fu_type_e decoded_fu_type_i,
    input  mem_cmd_e decoded_memory_cmd_i,
    input  logic     exu_completion_event_i,
    input  logic     lsu_completion_event_i,
    input  logic     instruction_retirement_event_i,
    ...
    input  logic pipeline_control_flush_event_i,
    input  logic pipeline_execute_instruction_discarded_i,
    input  logic fetch_taken_prediction_event_i,
    input  logic execute_misprediction_redirect_event_i,

    // 控制流结果在 EX 真正完成握手时采样；按条件分支、JAL、JALR 分别统计。
    // EX 预测纠错单独计数，不能用译码数量或重定向数量代替实际执行数量。
    input  logic             control_flow_resolution_event_i,
    ...
    input  logic     lsu_request_event_i,
    input  mem_cmd_e lsu_request_memory_cmd_i,
    input  logic     lsu_read_address_event_i,
    input  logic     lsu_read_response_event_i,
    input  logic     lsu_write_address_event_i,
    input  logic     lsu_write_data_event_i,
    input  logic     lsu_write_response_event_i,
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      if (!instruction_decode_event_i) begin
        if (pipeline_control_flush_event_i || control_recovery_pending_q) begin
          ...
          end else if (icache_miss_service_pending_q || icache_miss_event_i) begin
            frontend_cache_miss_service_stall_cycle_count_q <=
                frontend_cache_miss_service_stall_cycle_count_q + 64'd1;
          end else if (ifu_response_pending_q || ifu_instruction_request_event_i) begin
      ...
      if (pipeline_control_flush_event_i) begin
      ...
      if (fetch_taken_prediction_event_i) begin
        fetch_taken_prediction_count_q <= fetch_taken_prediction_count_q + 64'd1;
      end
      if (execute_misprediction_redirect_event_i) begin
  ...
  // 按进入 ID/EX 的握手统计指令类别；仅有 valid 不增加计数。
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
  ...
  // 候选静态预测策略：使用实际完成的控制流结果，独立于 RTL 预测器。
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      if (control_flow_resolution_event_i) begin
        unique case (resolved_control_flow_op_i)
          CF_BRANCH: begin
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      for (int unsigned entry_index = 0; entry_index < 16; entry_index++) begin
        bht_16_counter_array_q[entry_index] <= 2'b01;
      end
      for (int unsigned entry_index = 0; entry_index < 64; entry_index++) begin
        bht_64_counter_array_q[entry_index] <= 2'b01;
      end
    end else if (control_flow_resolution_event_i &&
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    ...
    end else if (ifu_instruction_response_event_i) begin
      icache_miss_service_pending_q <= 1'b0;
    end else if (icache_miss_event_i) begin
  ...
  logic        instruction_completion_event;

  assign instruction_completion_event = exu_completion_event_i || lsu_completion_event_i;
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      if (instruction_execution_active_q && instruction_completion_event) begin
      ...
      if (instruction_decode_event_i) begin
        ...
        active_instruction_elapsed_cycle_count_q <= '0;
                   ...
                   instruction_completion_event) begin
        instruction_execution_active_q           <= 1'b0;
        active_instruction_fu_type_q             <= FU_NONE;
        active_instruction_elapsed_cycle_count_q <= '0;
  ...
  // IFU 分别统计请求反压、响应等待和下游反压；这三项可能重叠，不可直接相加。
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      if (ifu_instruction_response_event_i) begin
      ...
      unique case ({ifu_instruction_request_event_i,
                    ifu_instruction_response_event_i})
        2'b10: begin
  ...
  // LSU 延迟从接收请求到结果握手，包含互联、外设等待和完成端反压。
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
    ...
    end else if (lsu_request_event_i && lsu_completion_event_i) begin
      ...
      lsu_operation_elapsed_cycle_count_q <= '0;
      lsu_operation_active_q              <= 1'b0;
    end else if (lsu_request_event_i) begin
      lsu_request_count_q                 <= lsu_request_count_q + 64'd1;
      active_lsu_memory_cmd_q             <= lsu_request_memory_cmd_i;
      lsu_operation_elapsed_cycle_count_q <= '0;
      lsu_operation_active_q              <= 1'b1;
    end else if (lsu_operation_active_q && lsu_completion_event_i) begin
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
    ...
    end else if (lsu_read_address_event_i && lsu_read_response_event_i) begin
      ...
      lsu_read_bus_elapsed_cycle_count_q <= '0;
      lsu_read_response_pending_q        <= 1'b0;
    end else if (lsu_read_address_event_i) begin
      lsu_read_bus_elapsed_cycle_count_q <= '0;
      lsu_read_response_pending_q        <= 1'b1;
    end else if (lsu_read_response_pending_q && lsu_read_response_event_i) begin
      lsu_read_bus_transaction_count_q <= lsu_read_bus_transaction_count_q + 64'd1;
  ...
  logic        lsu_write_request_channels_complete;

  assign lsu_write_request_channels_complete =
      (lsu_write_address_handshake_occurred_q || lsu_write_address_event_i) &&
      (lsu_write_data_handshake_occurred_q || lsu_write_data_event_i);

  // AW 和 W 独立握手；两者都完成后开始等待 B 响应，历史位记录先到的通道。
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      if (lsu_write_address_event_i) begin
        lsu_write_address_handshake_occurred_q <= 1'b1;
      end
      if (lsu_write_data_event_i) begin
        lsu_write_data_handshake_occurred_q <= 1'b1;
      end

      if (!lsu_write_response_pending_q && lsu_write_request_channels_complete) begin
      ...
      if (lsu_write_response_event_i) begin
