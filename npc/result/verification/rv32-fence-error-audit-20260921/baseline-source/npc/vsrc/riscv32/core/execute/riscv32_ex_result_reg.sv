...
// 校验前端预测并生成 redirect；寄存结果送回操作数准备模块，在 ID/EX 入口前递。
module riscv32_ex_result_reg
  import riscv32_pkg::*;
(
    input  logic clk_i,
    input  logic rst_ni,
...
`ifndef SYNTHESIS
  ...
  assert property (@(posedge clk_i) disable iff (!rst_ni)
    ...
    $error("EX/MEM stage changed result while backpressured");

  assert property (@(posedge clk_i) disable iff (!rst_ni)
    flush_i |=> !resolved_result_valid_o)
  else
    $error("EX/MEM stage retained a result after pipeline recovery");
