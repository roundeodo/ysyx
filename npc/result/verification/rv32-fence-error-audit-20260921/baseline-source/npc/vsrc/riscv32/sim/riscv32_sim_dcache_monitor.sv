...
module riscv32_sim_dcache_monitor
  import riscv32_pkg::*;
(
    input  logic clk_i,
    input  logic rst_ni,
  ...
  assign lookup_response_available_for_active_request =
      ...
      (lookup_timer_active_q || dcache_event_i.lookup_event);
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      store_hit_and_next_lookup_count_q <= store_hit_and_next_lookup_count_q +
          64'(dcache_event_i.store_hit_and_next_lookup_event);
      dirty_victim_miss_count_q <= dirty_victim_miss_count_q +
          64'(dcache_event_i.dirty_victim_miss_event);
      refill_request_count_q     <= refill_request_count_q + 64'(dcache_event_i.refill_request_event);
      refill_word_count_q        <= refill_word_count_q + 64'(dcache_event_i.refill_word_event);
      refill_transaction_count_q <= refill_transaction_count_q +
          64'(dcache_event_i.refill_transaction_completed_event);
      line_install_count_q      <= line_install_count_q + 64'(dcache_event_i.line_install_event);
      writeback_request_count_q <= writeback_request_count_q +
          64'(dcache_event_i.writeback_request_event);
      writeback_response_count_q <= writeback_response_count_q +
          64'(dcache_event_i.writeback_response_event);

      if (lookup_timer_active_q) begin
        lookup_elapsed_cycle_count_q <= lookup_elapsed_cycle_count_q + 64'd1;
      end

      if (dcache_event_i.miss_event) begin
      ...
      if (dcache_event_i.dirty_victim_miss_event) begin
      ...
      if (lookup_response_available_for_active_request) begin
        if (dcache_event_i.lookup_response_is_cache_hit) begin
          ...
          hit_latency_cycle_sum_q <= hit_latency_cycle_sum_q + lookup_elapsed_cycle_count_q + 64'd1;
          if (active_lookup_is_store_q) begin
            store_hit_count_q <= store_hit_count_q + 64'd1;
          end else begin
            load_hit_count_q <= load_hit_count_q + 64'd1;
          end
        end else if (lookup_classified_as_miss_q || dcache_event_i.miss_event) begin
          ...
          if (active_miss_has_dirty_victim_q || dcache_event_i.dirty_victim_miss_event) begin
      ...
      if (dcache_event_i.lookup_event) begin
  ...
  function automatic real calculate_miss_penalty();
    ...
    return calculate_average_cycles(
        ...
        hit_latency_cycle_sum_q, hit_count_q
    );
  endfunction
  ...
  final begin
    if (lookup_count_q != 0) begin
      ...
      $display("lookups / hits / misses                    : %0d / %0d / %0d", lookup_count_q,
               hit_count_q, miss_count_q);
      $display("D-cache hit rate                           : %.3f%%", calculate_percentage(
               hit_count_q, hit_count_q + miss_count_q));
      $display("load lookups / hits / misses               : %0d / %0d / %0d", load_lookup_count_q,
               load_hit_count_q, load_miss_count_q);
      ...
      $display("dirty victim misses                        : %0d (%.3f%% of misses)",
               dirty_victim_miss_count_q, calculate_percentage(dirty_victim_miss_count_q,
                                                               miss_count_q));
  ...
  always_ff @(posedge clk_i) begin
    if (rst_ni) begin
      assert ((hit_count_q + miss_count_q) <= lookup_count_q)
        ...
        $error("D-cache classified more requests than it accepted");

      assert ((load_lookup_count_q + store_lookup_count_q) == lookup_count_q)
      else
        $error("D-cache load/store request classification is inconsistent");

      assert ((load_hit_count_q + store_hit_count_q) == hit_count_q)
      else
        $error("D-cache load/store hit classification is inconsistent");

      assert ((load_miss_count_q + store_miss_count_q) == miss_count_q)
      else
        $error("D-cache load/store miss classification is inconsistent");

      assert (dirty_victim_miss_count_q <= miss_count_q)
      else
        $error("D-cache observed more dirty victims than demand misses");

      assert (miss_response_count_q <= miss_count_q)
      else
        $error("D-cache returned more miss responses than allocated misses");

      assert ((clean_miss_response_count_q + dirty_miss_response_count_q) == miss_response_count_q)
      else
        $error("D-cache clean/dirty miss response classification is inconsistent");

      assert (line_install_count_q <= refill_transaction_count_q)
      else
        $error("D-cache installed more lines than completed refill transactions");

      assert (!(dcache_event_i.lookup_event && lookup_timer_active_q &&
                !dcache_event_i.lookup_response_present))
      else
        $error("D-cache monitor observed more than one outstanding lookup");

      assert (!dcache_event_i.miss_event || lookup_timer_active_q || dcache_event_i.lookup_event)
      else
        $error("D-cache monitor observed a miss without an active lookup");
    end
  end
