...
spec = importlib.util.spec_from_file_location('compare',
    root / 'npc/result/performance/rv32-readability-ppa-20260919/compare.py')
module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)
timing = module.timing(target)
(target / 'timing.json').write_text(json.dumps(timing, indent=2) + '\n')
print(json.dumps(timing, indent=2))
assert all(group['slack_ns'] >= 0 for group in timing.values()) and 'slack (VIOLATED)' not in (target / 'riscv32_core_reset_boundary.rpt').read_text(), 'Timing does not close (check unrounded violation labels too)'
