...
module riscv32_dcache_miss_contract_tb;
  ...
  logic writeback_valid, writeback_ready = 0;
  logic writeback_response_valid = 0, writeback_response_ready;
  ...
  riscv32_dcache_miss_unit dut (
      ...
      .rst_ni(rst_n),
      .miss_req_i(request),
      .miss_req_valid_i(request_valid),
      .miss_req_ready_o(request_ready),
      .miss_resp_o(response),
      .miss_resp_valid_o(response_valid),
      .miss_resp_ready_i(response_ready),
      .clean_req_valid_i(clean_valid),
      .clean_req_ready_o(clean_ready),
      .clean_set_index_i(dcache_set_index_t'(1)),
      .clean_way_index_i(dcache_way_index_t'(0)),
      .clean_tag_i(dcache_tag_t'(3)),
      .clean_done_o(clean_done),
      .clean_access_fault_o(clean_fault),
      .victim_read_enable_o(victim_read),
      .victim_read_set_index_o(victim_set),
      .victim_read_word_index_o(victim_word),
      .victim_read_word_data_array_i(victim_data),
      .data_write_valid_o(data_write),
      .data_write_set_index_o(data_set),
      .data_write_way_index_o(data_way),
      .data_write_word_index_o(data_word),
      .data_write_word_data_o(data_value),
      .data_write_byte_strobe_o(data_strobe),
      .metadata_write_valid_o(metadata_write),
      .metadata_write_set_index_o(metadata_set),
      .metadata_write_way_index_o(metadata_way),
      .metadata_write_tag_o(metadata_tag),
      .metadata_write_line_present_o(metadata_present),
      .metadata_write_line_dirty_o(metadata_dirty),
      .line_install_event_o(installed),
      .installed_set_index_o(installed_set),
      .installed_way_index_o(installed_way),
      .transaction_present_o(busy),
      .refill_req_o(refill_request),
      .refill_req_valid_o(refill_valid),
      .refill_req_ready_i(refill_ready),
      .refill_resp_i(refill_response),
      .refill_resp_valid_i(refill_response_valid),
      .refill_resp_ready_o(refill_response_ready),
      .writeback_line_data_o(writeback_line),
      .writeback_req_o(writeback_request),
      .writeback_req_valid_o(writeback_valid),
      .writeback_req_ready_i(writeback_ready),
      .writeback_resp_i(writeback_response),
      .writeback_resp_valid_i(writeback_response_valid),
  ...
  int refill_accepts = 0, writeback_accepts = 0;
  int capture_waits = 0, refill_waits = 0;
  bit saved_line_present = 0;
  dcache_line_data_t saved_line;
  always @(posedge clk) begin
    if (rst_n) begin
      if (refill_valid && refill_ready) refill_accepts++;
      if (writeback_valid && writeback_ready) writeback_accepts++;
      if (dut.state_q == dut.MISS_CAPTURE_VICTIM_WORD && dut.victim_last_word) begin
        assert (writeback_valid)
          else $fatal(1, "last victim word did not bypass");
        if (writeback_ready && !writeback_valid) capture_waits++;
      end
      if (writeback_valid && writeback_ready && !dut.clean_operation_q) begin
        if (refill_ready && !refill_valid) refill_waits++;
      end
      if (writeback_valid && !saved_line_present) begin
        saved_line         = writeback_line;
        saved_line_present = 1;
      end
      if (saved_line_present)
        assert (writeback_line === saved_line)
        else $fatal(1, "line changed before B completion");
      if (writeback_response_valid && writeback_response_ready) saved_line_present = 0;
      if (dut.clean_operation_q && busy)
        assert (!refill_valid)
        else $fatal(1, "clean incorrectly requested refill");
    end
  end

  task automatic accept_writeback(input int stall_cycles = 0);
    writeback_ready = stall_cycles == 0;
    #1;
  ...
  task automatic check_miss(input bit dirty, input int b_order, input bit b_fault,
                            ...
                            input bit early_refill = 0, input int writeback_stall = 0);
    int before_responses, before_installs, before_refills, before_writebacks;
    bit expected_fault;
    data_memory_resp_t held;
    before_refills    = refill_accepts;
    before_writebacks = writeback_accepts;
    ...
    refill_ready  = 0;
    if (dirty) begin
      refill_ready = early_refill;
      accept_writeback(writeback_stall);
      if (refill_accepts == before_refills) begin
    ...
    for (int word_index = 0; word_index < DCACHE_WORDS_PER_LINE; word_index++) begin
      ...
      if (refill_response.last_word && (!dirty || b_order != 2) && !(dirty && expected_fault)) begin
        ...
        else $fatal(1, "miss completed before all required bus responses");
      end
      @(negedge clk);
      refill_response_valid    = 0;
      writeback_response_valid = 0;
    end
    if (dirty && b_order == 2) begin
      repeat (2) begin
        #1;
        assert (!response_valid && installs == before_installs)
    ...
    if (stall_response) begin
      ...
      repeat (3) begin
        @(negedge clk);
        assert (response_valid && response === held)
        else $fatal(1, "completion changed under stall");
      end
      response_ready = 1;
      @(negedge clk);
    end
    ...
    assert (!busy)
    else $fatal(1, "completed miss kept transaction busy");
    assert(refill_accepts==before_refills+1 &&
           writeback_accepts==before_writebacks+int'(dirty))
    else $fatal(1, "request lost or repeated");
    cases++;
  endtask

  task automatic check_clean(input bit fault, input int writeback_stall = 0);
    int before_done;
    before_done = clean_completions;
    @(negedge clk);
    clean_valid = 1;
    #1;
    assert (clean_ready && victim_read)
    else $fatal(1, "clean did not start victim read immediately");
    @(negedge clk);
    clean_valid  = 0;
    refill_ready = 1;
    accept_writeback(writeback_stall);
    ...
    @(negedge clk);
    writeback_response_valid = 0;
    assert (!busy && clean_completions == before_done + 1)
    else $fatal(1, "clean completion repeated");
    cases++;
  endtask
  ...
  initial begin
    ...
    for (int early = 0; early < 2; early++) begin
      for (int wait_cycles = 0; wait_cycles <= 3; wait_cycles += 3) begin
        for (int order = 0; order < 3; order++) begin
          for (int stall = 0; stall < 2; stall++) begin
            check_miss(1, order, 0, -1, DCACHE_WORDS_PER_LINE - 1, 1'(stall), 1'(early),
                       wait_cycles);
            check_miss(1, order, 1, -1, 0, 1'(stall), 1'(early), wait_cycles);
            check_miss(1, order, 0, DCACHE_WORDS_PER_LINE - 1, 0, 1'(stall), 1'(early),
                       wait_cycles);
          end
        end
        check_clean(0, wait_cycles);
        check_clean(1, wait_cycles);
      end
    end
    $display("BOUNDARIES capture_waits=%0d refill_waits=%0d", capture_waits, refill_waits);
    $display(
        "PASS D-cache miss contract: %0d cases, R/B order, faults, response stalls, allocation bypass, clean",
        cases);
    $finish;
  end
  initial begin
    #1000000;
    $fatal(1, "D-cache miss contract timeout");
  end
endmodule
