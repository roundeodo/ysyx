...
static int decode_exec(Decode *s) {
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli  , I, R(rd) = src1 >> BITS(s->isa.inst, 24, 20));
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu , I, R(rd) = (src1 < imm));
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori   , I, R(rd) = src1 | imm);
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori  , I, R(rd) = src1 ^ imm);
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti  , I, R(rd) = ((int32_t)src1 < (int32_t)imm));
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli  , I, R(rd) = src1 << BITS(s->isa.inst, 24, 20));
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai  , I, R(rd) = (int32_t)src1 >> BITS(s->isa.inst, 24, 20));
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr, I,
          ...
          if (rd == 1) FTRACE_CALL(s->pc, target);
          else if (rd == 0 && rs1_idx == 1 && imm == 0) FTRACE_RET(s->pc);
          R(rd) = t;
          s->dnpc = target;
        );
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div   , R, \
    if (src2 == 0) R(rd) = -1; \
    else if ((int32_t)src1 == INT32_MIN && (int32_t)src2 == -1) R(rd) = src1; \
    else R(rd) = (int32_t)src1 / (int32_t)src2);

  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem   , R, \
    if (src2 == 0) R(rd) = src1; \
    else if ((int32_t)src1 == INT32_MIN && (int32_t)src2 == -1) R(rd) = 0; \
    else R(rd) = (int32_t)src1 % (int32_t)src2);
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw, I, word_t csr_addr = BITS(s->isa.inst, 31, 20); word_t *csr = csr_decode(csr_addr); word_t old = *csr; *csr = src1; if (rd != 0) { R(rd) = old; });
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs, I, word_t csr_addr = BITS(s->isa.inst, 31, 20); word_t *csr = csr_decode(csr_addr); word_t old = *csr; if (BITS(s->isa.inst, 19, 15) != 0) { *csr = old | src1; } if (rd != 0) { R(rd) = old; });
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall, N, s->dnpc = isa_raise_intr(11, s->pc));
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret,  N, etrace_mret(s->pc); s->dnpc = cpu.mepc);
